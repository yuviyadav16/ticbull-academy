...
def send_otp_email(to_email, otp, is_delete=False):
    ...
    if is_delete:
        ...
        body = f"WARNING!\n\nYou have requested to PERMANENTLY DELETE your account. Your Deletion OTP is: {otp}\n\nTicBull Support"
    else:
...
def send_delete_otp():
    ...
    otp = str(random.randint(100000, 999999))
    otp_store[email] = otp
    if send_otp_email(email, otp, is_delete=True): return jsonify({"success": True, "message": f"Deletion Warning OTP sent"})
...
def chat():
    ...
    # 2. GEMINI AI CALL
    system_instruction = f"You are TicBull Teacher. Developed by MrYuviYadav. Student: {student_name}. Plan: {purchased_plan}. Course: {board} {cls} {stream} in {lang}. Teach clearly in bullet points. Do not mention Google or Gemini."
    try:
        headers = {"Content-Type": "application/json"}
        payload = {"contents": [{"parts": [{"text": f"{system_instruction}\n\nQuestion: {prompt}"}]}]}
        res_data = requests.post(f"https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key={GEMINI_API_KEY}", json=payload, headers=headers, timeout=25).json()
        
        if "candidates" in res_data:
            reply = res_data['candidates'][0]['content']['parts'][0]['text'].replace("Gemini", "TicBull").replace("Google", "TicBull")
        ...
        return jsonify({"success": False, "message": "AI Engine Error."}), 500
...
def admin_data():
    ...
    if FIREBASE_URL:
        ...
        for email_key, udata in users.items():
            ...
            user_list.append({"email": email_real, "name": udata.get("name", "Unknown"), "joined": udata.get("join_date", "Old User")})
...
@app.route('/')
def home(): return "TicBull Master AI Engine is securely running with API Rate Limiting!"
